...
- **Stock diagnostics and recovery** — capability-reported ARP entries, pf tables, configuration-history revisions, system halt, expanded service and package logs, and a separately unlocked command console are now available from the System screen. Runtime deletions, table flushes, backup removal, rollback aliases and halt require explicit confirmation; command output and history records are redacted before display, and read-only profiles expose only reported non-mutating operations.
