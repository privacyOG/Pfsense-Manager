...
class _FirewallRuleFormScreenState extends State<FirewallRuleFormScreen> {
  ...
  late final _dst = TextEditingController(
    text: widget.rule?.destinationNetwork ?? '*',
  );
  late final _from = TextEditingController(
    text: widget.rule?.destinationPortFrom?.toString() ?? '',
  );
  late final _to = TextEditingController(
    text: widget.rule?.destinationPortTo?.toString() ?? '',
  );
  ...
  Future<void> _save() async {
    ...
    if (!s.connected || s.service == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l?.disconnectedMessage ?? 'Disconnected')),
      );
      return;
    }
    ...
    final r = FirewallRule(
      ...
      enabled: _enabled,
      createdTime:
          widget.rule?.createdTime ?? DateTime.now().toIso8601String(),
    );
    try {
      if (r.id == null || r.id!.isEmpty) {
        await s.service!.createFirewallRule(r.toJson());
      } else {
        await s.service!.updateFirewallRule(r.id!, r.toJson());
      }
        ...
        child: ListView(
          ...
          children: [
            SegmentedButton<String>(
              selected: {_type},
              segments: [
                ButtonSegment(value: 'pass', label: Text(l?.pass ?? 'Pass')),
                ButtonSegment(value: 'block', label: Text(l?.block ?? 'Block')),
                ButtonSegment(
                    value: 'reject', label: Text(l?.reject ?? 'Reject')),
              ],
