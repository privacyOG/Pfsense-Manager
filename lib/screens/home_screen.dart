...
import 'gateway_history_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(l10n?.appTitle ?? 'pfSense Manager'),
        actions: [
          IconButton(
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: profiles.profiles.isEmpty
                  ? FilledButton.icon(
                      onPressed: () => Navigator.of(context).push(
                        MaterialPageRoute(
                  ...
                  : DropdownButtonFormField<String>(
                      ...
                      decoration: InputDecoration(
                        labelText: l10n?.profiles ?? 'Profiles',
                      ...
                      onChanged: (id) async {
                        ...
                        if (!mounted) return;
                        final current =
                            context.read<PfSenseSessionProvider>();
            ...
            if (session.connectionError != null)
              MaterialBanner(
                content: Text(session.connectionError!),
                actions: [
                  TextButton(
                    onPressed:
                        context.read<PfSenseSessionProvider>().clearError,
                    child: Text(l10n?.cancel ?? 'Dismiss'),
                  ),
                ],
              ),
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: [
          ...
          NavigationDestination(
            ...
            label: l10n?.firewallRules ?? 'Rules',
          ),
          NavigationDestination(
            icon: const Icon(Icons.article_outlined),
            label: l10n?.logs ?? 'Logs',
          ),
          NavigationDestination(
